...
export default async function PadDesaPage() {
  let padData = getPageContentStore().padDesa;

  if (prisma) {
    try {
      const record = await (prisma as any).pageContent?.findUnique({ where: { key: 'padDesa' } });
      if (record && record.content) {
        padData = { ...padData, ...JSON.parse(record.content) };
      }
    } catch (e) {}
  }
    ...
    <div className="flex flex-col min-h-screen bg-slate-50 text-slate-800">
      ...
      <section className="relative w-full py-20 px-4 md:px-10 bg-slate-900 overflow-hidden">
        <div className="absolute inset-0 z-0 opacity-20">
          <img
            src={padData.imageUrl || 'https://images.unsplash.com/photo-1554224155-8d04cb21cd6c?q=80&w=1200&auto=format&fit=crop'}
            alt={padData.title}
