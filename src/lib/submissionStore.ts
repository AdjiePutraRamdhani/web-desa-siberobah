...
// Initial seed data for demonstration purposes
let submissionsMemory: ServiceSubmissionItem[] = [
  {
    id: 'sub-101',
...
export function deleteSubmissionStore(id: string): boolean {
  const initialLen = submissionsMemory.length;
  submissionsMemory = submissionsMemory.filter((s) => s.id !== id);
  return submissionsMemory.length < initialLen;
}
